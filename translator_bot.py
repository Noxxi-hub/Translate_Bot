...
# ────────────────────────────────────────────────
# ... (der gesamte Groq-Call, detect_language_llm, translate_all bleibt gleich wie bei dir)
...
# (Der Rest von detect_language_llm und translate_all bleibt exakt wie in deiner Datei – ich habe sie hier aus Platzgründen nicht nochmal reingeschrieben)

# ────────────────────────────────────────────────
# FLAGGEN & SPRACHNAMEN – ERWEITERT
# ────────────────────────────────────────────────
...
# WICHTIG: Alle Sprachen, die dieser Bot übersetzen darf (inkl. DE + FR bei Raumsprachen)
ALL_LANGS = [
...
async def on_ready():
    ...
    if BOT_LOG_CHANNEL_ID:
        ...
        if channel:
            msg = "✅ **Übersetzer-Bot erfolgreich gestartet!**" if not errors else "⚠️ **Übersetzer-Bot gestartet mit Fehlern:**\n" + "\n".join(errors)
...
async def cmd_translate(ctx, action: str = None):
    ...
    if not action:
        await ctx.send("❓ Benutzung: `!ttranslate on` / `off` / `status`")
        return
    action = action.lower()
    if action == "on":
        translate_active = True
...
async def on_message(message: discord.Message):
    ...
    # GIF, YouTube, Stickers ignorieren
    if (any(a.filename.lower().endswith(".gif") or (a.content_type and "gif" in a.content_type.lower()) for a in message.attachments)
        or re.search(r'https?://\S*(?:tenor\.com|giphy\.com|youtube\.com|youtu\.be)\S*', message.content, re.IGNORECASE)
        or message.stickers):
        return
    ...
    # Raumsprachen prüfen (auch Forum-Parent)
    try:
    ...
    if room_setting is not None:
        if len(room_setting) == 0:
            return  # Kanal deaktiviert
        active_langs = room_setting
    else:
        active_langs = get_active_languages()

    # WICHTIG: Jetzt auch DE und FR erlauben, wenn sie im Raum aktiviert sind
    target_langs = [
